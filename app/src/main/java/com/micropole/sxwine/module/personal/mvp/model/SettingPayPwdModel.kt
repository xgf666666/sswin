 ...
 */
class SettingPayPwdModel : SettingPayPwdContract.Model {
    ...
    override fun alterOrSetPayPwd(mobile: String?, password: String?, password_confirmation: String?, verify_code: String?, httpObserver: HttpObserver<Any>?) {
        API.service.alterOrSettingPayPwd(mobile,password,password_confirmation,verify_code,(PreferencesHelper[Constants.USER_INFO, UserInfoEntity()] as UserInfoEntity).message_code)
                .compose(RxTransformer.io_main())
                .subscribe(httpObserver!!)
    }
}