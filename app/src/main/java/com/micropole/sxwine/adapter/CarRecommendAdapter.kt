 ...
 */
class CarRecommendAdapter(layoutId: Int, data: ArrayList<CarGoodsBean>, val isChoseAll: (isChoseAll: Boolean) -> Unit, val updateGoods: (id: Int, quantity: Int) -> Unit) : BaseQuickAdapter<CarGoodsBean, BaseViewHolder>(layoutId, data) {

    override fun convert(helper: BaseViewHolder, item: CarGoodsBean) {
        helper.getView<ImageView>(R.id.iv_icon).loadImg(mContext, API.HOST + item.goods.cover_img,transform = GlideRoundTransform())
