...
//                }
        ...
        mTvComplete.setOnClickListener(new MyOnClickListener());
...
//    };
    ...
    private void toggleSelection(View view, int position) {
        ...
        View v = view.findViewById(R.id.view_alpha);
        if (images.get(position).isSelected) {

            v.setAlpha(0.5f);
            ((FrameLayout) view).setForeground(this.getResources().getDrawable(R.drawable.ic_done_white));
...
//        adapter.notifyDataSetChanged();
    ...
    private void sendIntent() {
        ...
        ArrayList<Image> listSelected = getSelected();
        int size = listSelected.size();
        for (int i = 0; i < size; i++) {
            listImgs.add(listSelected.get(i).path);
        }
        Intent intent = new Intent();
//        intent.putParcelableArrayListExtra(Constants.INTENT_EXTRA_IMAGES, getSelected());
        intent.putStringArrayListExtra(Constants.INTENT_EXTRA_IMAGES, listImgs);
        ...
        public void run() {
            ...
            if (cursor.moveToLast()) {
                do {
                    ...
                    file = new File(path);
                    if (file.exists() && !path.endsWith(".gif")) {
    ...
    private void refreshStatus() {
        ...
        if (countSelected > 0) {
            ...
            mTvPreview.setAlpha(1.0f);

            mTvComplete.setClickable(true);
            mTvComplete.setAlpha(1.0f);
        } else {
            mTvPreview.setClickable(false);
            mTvPreview.setAlpha(0.5f);

            mTvComplete.setClickable(false);
            mTvComplete.setAlpha(0.5f);
        }

        //左下角显不已选图片数量
        mTvSelectedCount.setText(getString(R.string.text_select_picture_count, countSelected, Constants.limit));
    }
    ...
    private void goToPreviewActivity() {
        ...
        intent.putParcelableArrayListExtra(ImagePreviewActivity.IMGS_DATA_KEY, getSelected());
